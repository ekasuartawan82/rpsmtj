...
import { createRpsCpmk, createRpsCpmkSchema } from "@/services/rps";
...
export async function POST(request: Request, context: NestedRouteContext) {
  try {
    ...
    const payload = createRpsCpmkSchema.parse(await request.json());
    const cpmk = await createRpsCpmk(rpsId, payload, {
      actorUserId: session.user.id,
    });

    return NextResponse.json({ data: cpmk }, { status: 201 });
  } catch (error) {
